...
export async function getAdminRewards(): Promise<AdminReward[]> { const response = await fetch(`${apiBaseUrl}/api/admin/rewards`); if (!response.ok) throw new Error('Ödüller alınamadı.'); return response.json() as Promise<AdminReward[]> }
export async function createAdminReward(request: Omit<AdminReward, 'id' | 'createdAtUtc' | 'isAvailable'>): Promise<void> { const response = await fetch(`${apiBaseUrl}/api/admin/rewards`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(request) }); if (!response.ok) { const body = await response.json().catch(() => null) as { detail?: string } | null; throw new Error(body?.detail ?? 'Ödül oluşturulamadı.') } }
export async function updateAdminReward(reward: AdminReward): Promise<void> { const response = await fetch(`${apiBaseUrl}/api/admin/rewards/${reward.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(reward) }); if (!response.ok) throw new Error('Ödül güncellenemedi.') }
...
export async function getAdminLoyaltyRules(): Promise<LoyaltyRules> { const response = await fetch(`${apiBaseUrl}/api/admin/loyalty-rules`); if (!response.ok) throw new Error('Puan kuralları alınamadı.'); return response.json() as Promise<LoyaltyRules> }
export async function updateAdminLoyaltyRules(request: { silverThreshold: number; goldThreshold: number; pointsPerRewardTry: number; changeNote: string }): Promise<void> { const response = await fetch(`${apiBaseUrl}/api/admin/loyalty-rules`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(request) }); if (!response.ok) { const body = await response.json().catch(() => null) as { detail?: string } | null; throw new Error(body?.detail ?? 'Puan kuralları güncellenemedi.') } }
...
export async function getAdminLoyaltyReport(from: string, to: string): Promise<AdminLoyaltyReport> { const response = await fetch(`${apiBaseUrl}/api/admin/reports/loyalty?from=${from}&to=${to}`); if (!response.ok) throw new Error('Rapor alınamadı.'); return response.json() as Promise<AdminLoyaltyReport> }
export async function getReportExportAudits(): Promise<ReportExportAudit[]> { const response = await fetch(`${apiBaseUrl}/api/admin/reports/exports`); if (!response.ok) throw new Error('Dışa aktarma geçmişi alınamadı.'); return response.json() as Promise<ReportExportAudit[]> }
export async function exportAdminLoyaltyReport(from: string, to: string): Promise<void> { const response = await fetch(`${apiBaseUrl}/api/admin/reports/loyalty/export?from=${from}&to=${to}`); if (!response.ok) throw new Error('CSV oluşturulamadı.'); const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `usta-kulubu-raporu-${to}.csv`; link.click(); URL.revokeObjectURL(url) }
...
export async function getAdminSupportRequests(status?: string): Promise<AdminSupportRequest[]> { const query = status ? `?status=${status}` : ''; const response = await fetch(`${apiBaseUrl}/api/admin/support-requests${query}`); if (!response.ok) throw new Error('Destek talepleri alınamadı.'); return response.json() as Promise<AdminSupportRequest[]> }
export async function updateAdminSupportRequest(id: string, request: { status: string; priority: string; assignedTo: string | null }): Promise<void> { const response = await fetch(`${apiBaseUrl}/api/admin/support-requests/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(request) }); if (!response.ok) throw new Error('Destek talebi güncellenemedi.') }
export async function replyAdminSupportRequest(id: string, message: string): Promise<void> { const response = await fetch(`${apiBaseUrl}/api/admin/support-requests/${id}/responses`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message }) }); if (!response.ok) throw new Error('Yanıt gönderilemedi.') }
