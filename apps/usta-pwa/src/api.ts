...
export async function getDealerActivity(type?: string): Promise<DealerActivityResponse> { const query = type ? `?type=${encodeURIComponent(type)}` : ''; const response = await fetch(`${apiBaseUrl}/api/dealer/activity${query}`, { headers: dealerHeaders() }); const body = await response.json() as DealerActivityResponse & { message?: string }; if (!response.ok) throw new Error(body.message ?? 'Bayi işlem geçmişi alınamadı.'); return body }
...
export type AdminAuditEntry = { id: string; actor: string; action: string; entityType: string; entityId: string | null; details: string; createdAtUtc: string }
...
export type AdminOutboxItem = { id: string; type: string; status: 'Pending' | 'Delivered' | 'Failed'; attemptCount: number; createdAtUtc: string; nextAttemptAtUtc: string; deliveredAtUtc: string | null; lastError: string | null }
