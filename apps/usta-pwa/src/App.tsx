...
function Campaigns({ back }: { back: () => void }) {
  ...
  useEffect(() => { const controller = new AbortController(); setLoading(true); setError(''); setItems([]); getCampaigns(controller.signal).then((data) => { if (!controller.signal.aborted) setItems(data) }).catch((reason: unknown) => { if (!controller.signal.aborted) setError(reason instanceof Error ? reason.message : 'Kampanyalar yüklenemedi.') }).finally(() => { if (!controller.signal.aborted) setLoading(false) }); return () => controller.abort() }, [reloadKey])
...
function SupportRequestCard({ item, craftsmanId, onChanged }: { item: SupportItem; craftsmanId: string; onChanged: () => Promise<void> }) {
  ...
  async function send(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const response = reply.trim(); if (response.length < 3 || response.length > 1500) { setMessage('Yanıt 3 ile 1.500 karakter arasında olmalıdır.'); return } setBusy(true); setMessage(''); try { await replySupportRequest(craftsmanId, item.id, response); setReply(''); setMessage('Yanıtınız destek ekibine gönderildi.'); await onChanged() } catch (error) { setMessage(error instanceof Error ? error.message : 'Yanıt gönderilemedi.') } finally { setBusy(false) } }
...
function Support({ craftsmanId, back }: { craftsmanId: string; back: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const subjectText = subject.trim(); const descriptionText = description.trim(); const reference = referenceValue.trim().toUpperCase(); if (subjectText.length < 5 || subjectText.length > 140) { setMessage('Konu 5 ile 140 karakter arasında olmalıdır.'); return } if (descriptionText.length < 10 || descriptionText.length > 1500) { setMessage('Açıklama 10 ile 1.500 karakter arasında olmalıdır.'); return } if (category === 'İtiraz' && (reference.length < 4 || reference.length > 120)) { setMessage('İtiraz için geçerli bir işlem referansı girin.'); return } setSaving(true); setMessage(''); try { await createSupportRequest(craftsmanId, { category, subject: subjectText, description: descriptionText, referenceValue: reference || null }); setSubject(''); setDescription(''); setReferenceValue(''); setMessage(category === 'İtiraz' ? 'İtirazın işlem referansıyla kaydedildi.' : 'Destek talebin oluşturuldu.'); await load() } catch (error) { setMessage(error instanceof Error ? error.message : 'Talep oluşturulamadı.') } finally { setSaving(false) } }
...
function Login({ onAuthenticated }: { onAuthenticated: (result: { craftsmanId: string; needsProfile: boolean; token: string; expiresAtUtc: string }) => void }) {
  ...
  async function requestCode(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const digits = phone.replace(/\D/g, ''); const normalizedPhone = digits.length === 10 && digits.startsWith('5') ? `0${digits}` : digits; if (!/^05\d{9}$/.test(normalizedPhone)) { setMessage('Geçerli bir Türkiye cep telefonu numarası girin.'); return } setBusy(true); setMessage(''); try { const result = await requestOtpCode(normalizedPhone); setPhone(normalizedPhone); setChallengeId(result.id); setDevelopmentCode(result.developmentCode ?? ''); setMessage('6 haneli doğrulama kodu gönderildi.') } catch (error) { setMessage(error instanceof Error ? error.message : 'Kod gönderilemedi.') } finally { setBusy(false) } }
  async function verify(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!challengeId || !/^\d{6}$/.test(code)) { setMessage('6 haneli SMS kodunu eksiksiz girin.'); return } setBusy(true); setMessage(''); try { const result = await verifyOtpCode(challengeId, code); onAuthenticated(result) } catch (error) { setMessage(error instanceof Error ? error.message : 'Kod doğrulanamadı.') } finally { setBusy(false) } }
...
function DealerRiskForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const reference = referenceValue.trim().toUpperCase(); const trimmedReason = reason.trim(); const trimmedDescription = description.trim(); if (reference.length < 4 || reference.length > 120) { setMessage('İşlem referansı 4 ile 120 karakter arasında olmalıdır.'); return } if (trimmedReason.length < 3 || trimmedReason.length > 80) { setMessage('Şüphe nedeni 3 ile 80 karakter arasında olmalıdır.'); return } if (trimmedDescription.length < 10 || trimmedDescription.length > 1000) { setMessage('Açıklama 10 ile 1.000 karakter arasında olmalıdır.'); return } setBusy(true); setMessage(''); try { const result = await reportDealerRisk({ referenceType, referenceValue: reference, reason: trimmedReason, description: trimmedDescription }); setMessage(`Bildirim oluşturuldu · ${result.id.slice(0, 8).toUpperCase()}`); setReferenceValue(''); setReason(''); setDescription('') } catch (error) { setMessage(error instanceof Error ? error.message : 'Bildirim oluşturulamadı.') } finally { setBusy(false) } }
...
function AdminDealerEmployeesPage() {
  ...
  const [fullName, setFullName] = useState(''), [pin, setPin] = useState(''), [resetPins, setResetPins] = useState<Record<string, string>>({}), [message, setMessage] = useState(''), [busyId, setBusyId] = useState(''), [loading, setLoading] = useState(true), [loadError, setLoadError] = useState(''), [reloadKey, setReloadKey] = useState(0)
  const loadEmployees = async (id: string) => { if (!id) return; setEmployees(await getAdminDealerEmployees(id)) }
  useEffect(() => { setLoading(true); setLoadError(''); getAdminDealers().then(async (items) => { setDealers(items); const first = items.find((item) => item.isActive)?.id ?? items[0]?.id ?? ''; setDealerId(first); if (first) await loadEmployees(first) }).catch((error: unknown) => setLoadError(error instanceof Error ? error.message : 'Bayi listesi yüklenemedi.')).finally(() => setLoading(false)) }, [reloadKey])
  ...
  const toggle = async (item: AdminDealerEmployee) => { const action = item.isActive ? 'pasife almak ve açık oturumları kapatmak' : 'yeniden aktifleştirmek'; if (!window.confirm(`${item.fullName} adlı çalışanı ${action} istediğinize emin misiniz?`)) return; setBusyId(item.id); setMessage(''); try { await setAdminDealerEmployeeActive(dealerId, item.id, !item.isActive); setMessage(item.isActive ? 'Çalışan pasife alındı ve açık oturumları kapatıldı.' : 'Çalışan yeniden aktifleştirildi.'); await loadEmployees(dealerId) } catch (error) { setMessage(error instanceof Error ? error.message : 'Durum güncellenemedi.') } finally { setBusyId('') } }
  const resetPin = async (item: AdminDealerEmployee) => { const nextPin = resetPins[item.id] ?? ''; if (!/^\d{6}$/.test(nextPin)) { setMessage('Yeni erişim kodu tam olarak 6 rakam olmalıdır.'); return } if (!window.confirm(`${item.fullName} adlı çalışanın erişim kodu yenilenecek ve açık oturumları kapatılacak. Onaylıyor musunuz?`)) return; setBusyId(item.id); setMessage(''); try { await resetAdminDealerEmployeePin(dealerId, item.id, nextPin); setResetPins({ ...resetPins, [item.id]: '' }); setMessage('Erişim kodu yenilendi ve çalışanın açık oturumları kapatıldı.'); await loadEmployees(dealerId) } catch (error) { setMessage(error instanceof Error ? error.message : 'Kod yenilenemedi.') } finally { setBusyId('') } }
  return <main className="admin-shell"><aside><div className="admin-brand"><span>⚒</span><div><small>USTA KULÜBÜ</small><strong>Yönetim</strong></div></div><nav><a href="/admin">⌂ Genel Bakış</a><a href="/admin/dealers">▣ Bayiler</a><a className="active" href="/admin/dealer-employees">♧ Bayi Çalışanları</a></nav></aside><section className="admin-main"><header><div><span>YÖNETİCİ PANELİ</span><h1>Bayi Çalışanı Yönetimi</h1></div><b>{employees.filter((item) => item.isActive).length} aktif çalışan</b></header>{loading && <div className="coupon-state" aria-live="polite">Bayi ve çalışanlar yükleniyor…</div>}{loadError && !loading && <div className="screen-error" role="alert"><span>{loadError}</span><button onClick={() => setReloadKey((value) => value + 1)} type="button">Tekrar dene</button></div>}<label className="employee-dealer-select">Bayi<select disabled={loading || Boolean(loadError)} value={dealerId} onChange={(event) => chooseDealer(event.target.value)}>{dealers.map((dealer) => <option value={dealer.id} key={dealer.id}>{dealer.code} · {dealer.name}{dealer.isActive ? '' : ' (pasif)'}</option>)}</select></label>{message && <p className="admin-info-message">{message}</p>}<div className="employee-admin-layout"><form className="employee-create" onSubmit={create}><h2>Yeni çalışan</h2><p>Her çalışan için ayrı bir giriş kodu oluşturun. Kod veritabanında açık biçimde tutulmaz.</p><label>Ad soyad<input value={fullName} onChange={(event) => setFullName(event.target.value)} minLength={3} maxLength={120} required /></label><label>6 haneli erişim kodu<input value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" pattern="[0-9]{6}" maxLength={6} autoComplete="new-password" required /></label><button disabled={loading || Boolean(loadError) || busyId === 'new' || !dealerId} type="submit">{busyId === 'new' ? 'Oluşturuluyor…' : 'Çalışanı Oluştur'}</button></form><section className="employee-list">{employees.map((item) => <article key={item.id}><div className="employee-avatar">{item.fullName.split(' ').map((part) => part[0]).join('').slice(0, 2)}</div><div className="employee-copy"><h2>{item.fullName}</h2><p>{item.hasAccessCode ? 'Erişim kodu tanımlı' : 'Erişim kodu eksik'} · {item.activeSessions} açık oturum</p><span className={item.isActive ? 'entity-active' : 'entity-passive'}>{item.isActive ? 'Aktif' : 'Pasif'}</span></div><button onClick={() => toggle(item)} disabled={busyId === item.id} type="button">{item.isActive ? 'Pasife Al' : 'Aktifleştir'}</button><div className="employee-pin-reset"><input value={resetPins[item.id] ?? ''} onChange={(event) => setResetPins({ ...resetPins, [item.id]: event.target.value.replace(/\D/g, '').slice(0, 6) })} inputMode="numeric" placeholder="Yeni 6 haneli kod" maxLength={6} /><button onClick={() => resetPin(item)} disabled={busyId === item.id || (resetPins[item.id]?.length ?? 0) !== 6} type="button">Kodu Yenile</button></div></article>)}{!loading && !loadError && employees.length === 0 && <p>Bu bayide henüz çalışan bulunmuyor.</p>}</section></div></section></main>
}
...
function AdminCampaignApprovalsPage() {
  ...
  const [loading, setLoading] = useState(true), [loadError, setLoadError] = useState(''), [reloadKey, setReloadKey] = useState(0)
  const load = useCallback(async () => { setLoading(true); setLoadError(''); try { setItems(await getAdminCampaignApprovals()) } catch (error) { setLoadError(error instanceof Error ? error.message : 'Kampanya onayları yüklenemedi.') } finally { setLoading(false) } }, [])
...
function AdminCampaignsPage() {
  ...
  const [items, setItems] = useState<AdminCampaign[]>([]); const [products, setProducts] = useState<AdminProduct[]>([]); const [productId, setProductId] = useState(''); const [title, setTitle] = useState(''); const [summary, setSummary] = useState(''); const [multiplier, setMultiplier] = useState(2); const [startsAt, setStartsAt] = useState(initialStart); const [endsAt, setEndsAt] = useState(initialEnd); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false); const [loading, setLoading] = useState(true); const [loadError, setLoadError] = useState(''); const [reloadKey, setReloadKey] = useState(0)
  const load = useCallback(async () => { setLoading(true); setLoadError(''); try { const [campaigns, productItems] = await Promise.all([getAdminCampaigns(), getAdminProducts()]); setItems(campaigns); setProducts(productItems.filter((x) => x.isActive)) } catch (error) { setLoadError(error instanceof Error ? error.message : 'Kampanya verileri yüklenemedi.') } finally { setLoading(false) } }, [])
  useEffect(() => { void load() }, [load, reloadKey])
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const start = new Date(startsAt); const end = new Date(endsAt); if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end <= start) { setMessage('Bitiş tarihi başlangıç tarihinden sonra olmalıdır.'); return } if (!Number.isFinite(multiplier) || multiplier < 1 || multiplier > 10) { setMessage('Puan çarpanı 1 ile 10 arasında olmalıdır.'); return } setBusy(true); setMessage(''); try { const created = await createAdminCampaign({ title, summary, pointMultiplier: multiplier, startsAtUtc: start.toISOString(), endsAtUtc: end.toISOString(), isActive: true, displayOrder: 1, productId: productId || null }); setTitle(''); setSummary(''); setProductId(''); setMessage(created.requiresApproval ? 'Yüksek etkili kampanya ikinci yönetici onayına gönderildi; onaydan sonra etkinleşecek.' : 'Ürün ve tarih kuralı kod değişmeden yayınlandı.'); await load() } catch (error) { setMessage(error instanceof Error ? error.message : 'Kampanya oluşturulamadı.') } finally { setBusy(false) } }
  async function toggle(item: AdminCampaign) { setBusy(true); setMessage(''); try { await setAdminCampaignActive(item.id, !item.isActive); await load(); setMessage(item.isActive ? 'Kampanya durduruldu.' : 'Kampanya etkinleştirildi.') } catch (error) { setMessage(error instanceof Error ? error.message : 'Kampanya durumu güncellenemedi.') } finally { setBusy(false) } }
  return <main className="admin-shell"><aside><div className="admin-brand"><span>⚒</span><div><small>USTA KULÜBÜ</small><strong>Yönetim</strong></div></div><nav><a href="/admin">⌂ Genel Bakış</a><a href="/admin/craftsmen">♧ Ustalar</a><a href="/admin/dealers">▣ Bayiler</a><a className="active" href="/admin/campaigns">◇ Kampanyalar</a></nav></aside><section className="admin-main"><header><div><span>YÖNETİCİ PANELİ</span><h1>Kampanya Yönetimi</h1></div><b>{items.length} kampanya</b></header>{loading && <div className="coupon-state" aria-live="polite">Kampanyalar yükleniyor…</div>}{loadError && !loading && <div className="screen-error" role="alert"><span>{loadError}</span><button onClick={() => setReloadKey((value) => value + 1)} type="button">Tekrar dene</button></div>}<div className="campaign-admin-grid"><form className="campaign-create" onSubmit={submit}><h2>Yeni Kampanya</h2><label>Başlık<input value={title} onChange={(event) => setTitle(event.target.value)} minLength={3} maxLength={140} placeholder="Ağustos Çifte Puan" required /></label><label>Geçerli ürün<select value={productId} onChange={(event) => setProductId(event.target.value)}><option value="">Tüm aktif ürünler</option>{products.map((product) => <option value={product.id} key={product.id}>{product.sku} · {product.name}</option>)}</select><small>Tek ürün seçerseniz çarpan yalnızca o ürünün kodlarında çalışır.</small></label><label>Açıklama<textarea value={summary} onChange={(event) => setSummary(event.target.value)} minLength={10} maxLength={500} required /></label><label>Puan çarpanı<input value={multiplier} onChange={(event) => setMultiplier(Number(event.target.value))} type="number" min="1" max="10" step="0.1" required /></label><div><label>Başlangıç<input value={startsAt} onChange={(event) => setStartsAt(event.target.value)} type="datetime-local" required /></label><label>Bitiş<input value={endsAt} onChange={(event) => setEndsAt(event.target.value)} type="datetime-local" required /></label></div>{message && <p>{message}</p>}<button disabled={busy} type="submit">{busy ? 'Kaydediliyor…' : 'Kampanya Kuralını Yayınla'}</button></form><section className="admin-campaign-list">{items.map((item) => <article key={item.id}><div><span>{item.pointMultiplier}X</span><b className={item.isActive ? 'entity-active' : 'entity-passive'}>{item.isActive ? 'Aktif' : 'Pasif'}</b></div><h2>{item.title}</h2><p>{item.summary}</p><strong>{item.productName ?? 'Tüm aktif ürünler'}</strong><small>{new Intl.DateTimeFormat('tr-TR', { dateStyle: 'medium' }).format(new Date(item.startsAtUtc))} – {new Intl.DateTimeFormat('tr-TR', { dateStyle: 'medium' }).format(new Date(item.endsAtUtc))}</small><button onClick={() => toggle(item)} disabled={busy} type="button">{item.isActive ? 'Durdur' : 'Etkinleştir'}</button></article>)}{!loading && !loadError && items.length === 0 && <p className="coupon-state">Henüz kampanya tanımlanmamış.</p>}</section></div></section></main>
}

function AdminRewardCard({ initial, onSaved }: { initial: AdminReward; onSaved: () => Promise<void> }) {
  const [item, setItem] = useState(initial); const [saving, setSaving] = useState(false); const [feedback, setFeedback] = useState('')
  useEffect(() => setItem(initial), [initial])
  async function save() { setSaving(true); setFeedback(''); try { await updateAdminReward(item); await onSaved(); setFeedback('Ödül güncellendi.') } catch (error) { setFeedback(error instanceof Error ? error.message : 'Ödül kaydedilemedi.') } finally { setSaving(false) } }
  return <article><div className="reward-admin-art">{rewardArt[item.imageKey] ?? '🎁'}</div><div className="reward-admin-fields"><input value={item.name} onChange={(event) => setItem({ ...item, name: event.target.value })} minLength={3} maxLength={120} /><textarea value={item.description} onChange={(event) => setItem({ ...item, description: event.target.value })} minLength={5} maxLength={400} /><div><label>Puan<input value={item.pointCost} onChange={(event) => setItem({ ...item, pointCost: Number(event.target.value) })} type="number" min="1" /></label><label>Stok<input value={item.stockQuantity ?? ''} onChange={(event) => setItem({ ...item, stockQuantity: event.target.value === '' ? null : Number(event.target.value) })} type="number" min="0" placeholder="Sınırsız" /></label></div></div><label className="reward-active"><input checked={item.isActive} onChange={(event) => setItem({ ...item, isActive: event.target.checked })} type="checkbox" />Aktif</label>{feedback && <p className="admin-info-message" role="status">{feedback}</p>}<button onClick={save} disabled={saving} type="button">{saving ? 'Kaydediliyor…' : 'Kaydet'}</button></article>
}

function AdminRewardsPage() {
  const [items, setItems] = useState<AdminReward[]>([]); const [name, setName] = useState(''); const [description, setDescription] = useState(''); const [pointCost, setPointCost] = useState(1500); const [deliveryType, setDeliveryType] = useState<Reward['deliveryType']>('Digital'); const [stock, setStock] = useState(''); const [message, setMessage] = useState(''); const [busy, setBusy] = useState(false); const [loading, setLoading] = useState(true); const [loadError, setLoadError] = useState(''); const [reloadKey, setReloadKey] = useState(0)
  const load = useCallback(async () => { setLoading(true); setLoadError(''); try { setItems(await getAdminRewards()) } catch (error) { setLoadError(error instanceof Error ? error.message : 'Ödüller yüklenemedi.') } finally { setLoading(false) } }, [])
  useEffect(() => { void load() }, [load, reloadKey])
...
function ProductCodeImportPanel() {
  ...
  const [productsLoading, setProductsLoading] = useState(true), [productsError, setProductsError] = useState(''), [reloadKey, setReloadKey] = useState(0)
  const loadProducts = useCallback(async () => { setProductsLoading(true); setProductsError(''); try { const items = await getAdminProducts(); const active = items.filter((item) => item.isActive); setProducts(active); setProductId(active[0]?.id ?? '') } catch { setProductsError('Ürün listesi alınamadı.') } finally { setProductsLoading(false) } }, [])
  useEffect(() => { void loadProducts() }, [loadProducts, reloadKey])
  const readFile = async (file?: File) => { setValidation(null); setMessage(''); setCodes([]); if (!file) return; if (file.size > 1024 * 1024) { setMessage('Dosya en fazla 1 MB olabilir.'); return } try { const text = await file.text(); const values = text.split(/[\r\n,;]+/).map((value) => value.trim()).filter(Boolean); if (values[0] && ['code', 'kod', 'product_code', 'urun_kodu'].includes(values[0].toLowerCase())) values.shift(); if (values.length > 5000) { setMessage('Tek dosyada en fazla 5.000 kod yüklenebilir.'); return } setFileName(file.name); setCodes(values); setMessage(`${values.length} kod yerel olarak okundu. Henüz veritabanına yazılmadı.`) } catch { setMessage('Dosya okunamadı. Lütfen tekrar seçin.') } }
  ...
  return <details className="product-import-panel"><summary>⇧ Kod Dosyası Yükle</summary><section><h2>Toplu ürün kodu yükleme</h2><p>TXT veya CSV dosyasında her satıra bir kod yazın. Virgül ve noktalı virgül de desteklenir.</p>{productsLoading && <p role="status">Ürün listesi yükleniyor…</p>}{productsError && <p className="admin-message" role="alert">{productsError} <button onClick={() => setReloadKey((value) => value + 1)} type="button">Tekrar Dene</button></p>}<label>Ürün<select value={productId} onChange={(event) => { setProductId(event.target.value); setValidation(null) }} disabled={productsLoading || Boolean(productsError)}>{products.map((product) => <option value={product.id} key={product.id}>{product.sku} · {product.name}</option>)}</select></label><label>Dosya<input onChange={(event) => void readFile(event.target.files?.[0])} disabled={productsLoading || Boolean(productsError)} type="file" accept=".txt,.csv,text/plain,text/csv" /></label>{fileName && <small>{fileName} · {codes.length} kod</small>}<div className="import-actions"><button onClick={validate} disabled={busy || productsLoading || Boolean(productsError) || !productId || codes.length === 0} type="button">Önce Doğrula</button><button className="import-confirm" onClick={upload} disabled={busy || productsLoading || Boolean(productsError) || !validation || validation.rejected > 0 || validation.valid === 0} type="button">{busy ? 'İşleniyor…' : `${validation?.valid ?? 0} Kodu Yükle`}</button></div>{message && <p className="import-message">{message}</p>}{validation && <div className="import-result"><div><span>Toplam<strong>{validation.total}</strong></span><span>Geçerli<strong>{validation.valid}</strong></span><span>Hatalı<strong>{validation.rejected}</strong></span></div>{validation.rejectedItems.length > 0 && <ul>{validation.rejectedItems.slice(0, 8).map((item) => <li key={`${item.line}-${item.maskedCode}`}>Satır {item.line} · {item.maskedCode}: {item.reason}</li>)}</ul>}</div>}<aside>Ham kodlar yalnızca doğrulama ve yükleme sırasında bellekte kullanılır. SQL Server'a yalnızca SHA-256 özeti kaydedilir.</aside></section></details>
}
...
function AdminLoyaltyRulesPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!Number.isInteger(silver) || silver < 1 || !Number.isInteger(gold) || gold <= silver) { setMessage('Altın seviye eşiği, geçerli Gümüş eşiğinden büyük bir tam sayı olmalıdır.'); return } if (!Number.isInteger(rate) || rate < 1 || rate > 1000) { setMessage('Ödül değeri için puan oranı 1 ile 1.000 arasında tam sayı olmalıdır.'); return } if (note.trim().length < 5) { setMessage('Değişiklik gerekçesi en az 5 karakter olmalıdır.'); return } setBusy(true); setMessage(''); try { await updateAdminLoyaltyRules({ silverThreshold: silver, goldThreshold: gold, pointsPerRewardTry: rate, changeNote: note.trim() }); setNote(''); setMessage('Puan ve seviye kuralları yayınlandı.'); await load() } catch (error) { setMessage(error instanceof Error ? error.message : 'Kurallar güncellenemedi.') } finally { setBusy(false) } }
...
function AdminReportsPage() {
  ...
  const [report, setReport] = useState<AdminLoyaltyReport | null>(null), [audits, setAudits] = useState<ReportExportAudit[]>([]), [message, setMessage] = useState('')
  const [reportLoading, setReportLoading] = useState(true), [reportError, setReportError] = useState(''), [auditsLoading, setAuditsLoading] = useState(true), [auditsError, setAuditsError] = useState(''), [downloading, setDownloading] = useState(false)
  const loadReport = useCallback(async () => { setReportLoading(true); setReportError(''); try { setReport(await getAdminLoyaltyReport(from, to)) } catch (error) { setReportError(error instanceof Error ? error.message : 'Rapor yüklenemedi.') } finally { setReportLoading(false) } }, [from, to])
  const loadAudits = useCallback(async () => { setAuditsLoading(true); setAuditsError(''); try { setAudits(await getReportExportAudits()) } catch { setAuditsError('Dışa aktarma geçmişi yüklenemedi.') } finally { setAuditsLoading(false) } }, [])
  const load = useCallback(async () => { setMessage(''); await Promise.all([loadReport(), loadAudits()]) }, [loadAudits, loadReport])
  useEffect(() => { void load() }, [load])
  const download = async () => { setDownloading(true); setMessage(''); try { await exportAdminLoyaltyReport(from, to); setMessage('Maskeli CSV raporu indirildi ve işlem denetim kaydına eklendi.'); void loadAudits() } catch (error) { setMessage(error instanceof Error ? error.message : 'Rapor indirilemedi.') } finally { setDownloading(false) } }
  const maxEarned = Math.max(...(report?.daily.map((item) => item.earned) ?? [1]), 1)
  return <main className="admin-shell"><aside><div className="admin-brand"><span>⚒</span><div><small>USTA KULÜBÜ</small><strong>Yönetim</strong></div></div><nav><a href="/admin">⌂ Genel Bakış</a><a href="/admin/transactions">◴ İşlem Geçmişi</a><a className="active" href="/admin/reports">▥ Raporlar</a></nav></aside><section className="admin-main"><header><div><span>YÖNETİCİ PANELİ</span><h1>Sadakat Raporları</h1></div><b>Kişisel veriler maskeli</b></header><form className="report-toolbar" onSubmit={(event) => { event.preventDefault(); void load() }}><label>Başlangıç<input value={from} onChange={(event) => setFrom(event.target.value)} type="date" max={to} /></label><label>Bitiş<input value={to} onChange={(event) => setTo(event.target.value)} type="date" min={from} max={today} /></label><button disabled={reportLoading} type="submit">{reportLoading ? 'Yükleniyor…' : 'Raporu Getir'}</button><button className="export" onClick={download} disabled={downloading || !report} type="button">{downloading ? 'İndiriliyor…' : 'CSV İndir'}</button></form>{message && <p className="admin-info-message">{message}</p>}{reportError && <p className="admin-message" role="alert">{reportError} <button onClick={() => void loadReport()} type="button">Tekrar Dene</button></p>}<div className="report-stats"><article><small>Kazanılan</small><strong>+{numberFormatter.format(report?.summary.earnedPoints ?? 0)}</strong></article><article><small>Harcanan</small><strong>{numberFormatter.format(report?.summary.spentPoints ?? 0)}</strong></article><article><small>Aktif Usta</small><strong>{report?.summary.uniqueCraftsmen ?? 0}</strong></article><article><small>Ödül Talebi</small><strong>{report?.summary.rewardRequests ?? 0}</strong></article><article><small>Teslim Edilen</small><strong>{report?.summary.fulfilledRewards ?? 0}</strong></article></div><div className="report-grid"><section className="report-panel"><h2>Günlük Puan Kazanımı</h2><div className="report-chart">{reportLoading && <p role="status">Rapor verisi yükleniyor…</p>}{!reportLoading && report?.daily.length === 0 && <p>Seçilen dönemde hareket bulunmuyor.</p>}{report?.daily.map((item) => <div key={item.date}><time>{new Intl.DateTimeFormat('tr-TR', { day: '2-digit', month: 'short' }).format(new Date(`${item.date}T00:00:00`))}</time><span><i style={{ width: `${Math.max(item.earned / maxEarned * 100, item.earned ? 3 : 0)}%` }} /></span><b>{numberFormatter.format(item.earned)}</b></div>)}</div></section><section className="report-panel"><h2>En Çok Puan Kazanan Ustalar</h2><div className="report-ranking">{report?.topCraftsmen.map((item, index) => <article key={`${item.name}-${index}`}><span>{index + 1}</span><div><b>{item.name}</b><small>{item.phoneNumber}</small></div><strong>{numberFormatter.format(item.earnedPoints)}</strong></article>)}</div></section><section className="report-panel"><h2>Ödül Tercihleri</h2><div className="report-ranking">{report?.topRewards.map((item) => <article key={item.name}><span>♙</span><div><b>{item.name}</b><small>{item.count} talep</small></div><strong>{numberFormatter.format(item.points)} puan</strong></article>)}</div></section><section className="report-panel"><h2>Dışa Aktarma Denetimi</h2><div className="report-ranking">{auditsLoading && <p role="status">Dışa aktarma geçmişi yükleniyor…</p>}{auditsError && <p className="admin-message" role="alert">{auditsError} <button onClick={() => void loadAudits()} type="button">Tekrar Dene</button></p>}{!auditsLoading && !auditsError && audits.length === 0 && <p>Henüz rapor indirilmedi.</p>}{audits.map((item) => <article key={item.id}><span>CSV</span><div><b>{item.actor}</b><small>{dateFormatter.format(new Date(item.createdAtUtc))}</small></div><strong>{item.rowCount} satır</strong></article>)}</div></section></div></section></main>
}
...
function AdminOutboxPage() {
  ...
  const load = useCallback(async () => { setLoading(true); setLoadError(''); try { setData(await getAdminOutbox()) } catch (error) { setLoadError(error instanceof Error ? error.message : 'Teslim kuyruğu yüklenemedi.') } finally { setLoading(false) } }, [])
...
function AdminCouponsPage() {
  ...
  const [loading, setLoading] = useState(true)
  const load = async (nextStatus = status, nextSearch = search) => { setLoading(true); setMessage(''); setData(null); try { setData(await getAdminCoupons(nextStatus || undefined, nextSearch.trim() || undefined)) } catch (error) { setMessage(error instanceof Error ? error.message : 'Kupon takibi yüklenemedi.') } finally { setLoading(false) } }
  useEffect(() => { void load('', '') }, [])
...
function AdminPointAdjustmentsPage() {
  ...
  const [craftsmenLoading, setCraftsmenLoading] = useState(true), [craftsmenError, setCraftsmenError] = useState(''), [reloadKey, setReloadKey] = useState(0)
  useEffect(() => { let active = true; setCraftsmenLoading(true); setCraftsmenError(''); getAdminCraftsmen().then((items) => { if (active) { const activeCraftsmen = items.filter((item) => item.isActive); setCraftsmen(activeCraftsmen); setCraftsmanId(activeCraftsmen[0]?.id ?? '') } }).catch(() => { if (active) setCraftsmenError('Usta listesi yüklenemedi.') }).finally(() => { if (active) setCraftsmenLoading(false) }); return () => { active = false } }, [reloadKey])
  ...
  return <main className="admin-shell"><aside><div className="admin-brand"><span>⚒</span><div><small>USTA KULÜBÜ</small><strong>Yönetim</strong></div></div><nav><a href="/admin">⌂ Genel Bakış</a><a href="/admin/transactions">◴ İşlem Geçmişi</a><a className="active" href="/admin/adjustments">± Puan Düzeltmesi</a><a href="/admin/support">☏ Destek</a></nav></aside><section className="admin-main"><header><div><span>YÖNETİCİ PANELİ</span><h1>Yetkili Puan Düzeltmesi</h1></div><b>Denetimli işlem</b></header><section className="adjustment-layout"><form className="adjustment-form" onSubmit={submit}><h2>Yeni düzeltme</h2><p>Yalnızca doğrulanmış destek veya operasyon kararlarında kullanın.</p>{craftsmenLoading && <p role="status">Usta listesi yükleniyor…</p>}{craftsmenError && <p className="admin-message" role="alert">{craftsmenError} <button onClick={() => setReloadKey((value) => value + 1)} type="button">Tekrar Dene</button></p>}<label>Usta<select value={craftsmanId} onChange={(event) => setCraftsmanId(event.target.value)} disabled={craftsmenLoading || Boolean(craftsmenError)} required>{craftsmen.map((item) => <option value={item.id} key={item.id}>{item.fullName} · {item.phoneNumber}</option>)}</select></label>{selected && <div className="adjustment-balance"><span>Mevcut bakiye</span><strong>{numberFormatter.format(selected.balance)} puan</strong></div>}<label>Düzeltme miktarı<input value={amount || ''} onChange={(event) => setAmount(Number(event.target.value))} type="number" min="-100000" max="100000" placeholder="Örn. 250 veya -250" required /><small>Puan eklemek için pozitif, geri almak için negatif yazın.</small></label><label>Karar ve gerekçe<textarea value={reason} onChange={(event) => setReason(event.target.value)} minLength={10} maxLength={240} placeholder="Destek talebi, kanıt ve karar gerekçesi" required /></label><label className="adjustment-confirm"><input checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} type="checkbox" /><span>Ustayı, miktarı ve gerekçeyi kontrol ettim. Bu işlemin geri alınması için yeni bir ters kayıt gerektiğini biliyorum.</span></label><button disabled={busy || craftsmenLoading || Boolean(craftsmenError) || !confirmed || amount === 0 || !craftsmanId} type="submit">{busy ? 'Kaydediliyor…' : 'Düzeltmeyi Kaydet'}</button>{message && <p className="admin-info-message">{message}</p>}</form><aside className="adjustment-rules"><h2>Neden silinmiyor?</h2><p>Puan hareketi finansal kayda benzer. Eski kayıt değiştirilmez; her düzeltme yeni ve izlenebilir bir hareket olarak eklenir.</p><ul><li>İşlemi yapan yönetici kayda eklenir.</li><li>Gerekçe yazmak zorunludur.</li><li>Ustaya uygulama içi bildirim gider.</li><li>Sonuç işlem geçmişinde görünür.</li></ul></aside></section></section></main>
}

function AdminTransactionsPage() {
  const [data, setData] = useState<AdminTransactionResponse | null>(null); const [filter, setFilter] = useState(''); const [message, setMessage] = useState(''); const [loading, setLoading] = useState(true); const [reloadKey, setReloadKey] = useState(0)
  useEffect(() => { let active = true; setLoading(true); setMessage(''); setData(null); getAdminTransactions(filter || undefined).then((result) => { if (active) setData(result) }).catch(() => { if (active) setMessage('İşlem geçmişi yüklenemedi.') }).finally(() => { if (active) setLoading(false) }); return () => { active = false } }, [filter, reloadKey])
...
function DealerPerformance({ refreshKey }: { refreshKey: number }) {
  const [data, setData] = useState<DealerDashboard | null>(null); const [error, setError] = useState(''); const [loading, setLoading] = useState(true); const [reloadKey, setReloadKey] = useState(0)
  useEffect(() => { const controller = new AbortController(); setLoading(true); setError(''); setData(null); getDealerDashboard(controller.signal).then((result) => { if (!controller.signal.aborted) setData(result) }).catch((reason: unknown) => { if (!(reason instanceof DOMException && reason.name === 'AbortError')) setError(reason instanceof Error ? reason.message : 'Bayi özeti yüklenemedi.') }).finally(() => { if (!controller.signal.aborted) setLoading(false) }); return () => controller.abort() }, [refreshKey, reloadKey])
  ...
  if (loading || !data) return <section className="dealer-performance loading">Bayi katkısı hesaplanıyor…</section>
