...
import { createAdminCampaign, createAdminProduct, createAdminReward, createDealerSale, createMembershipPass, createSupportRequest, exportAdminLoyaltyReport, fulfillDealerCoupon, generateAdminProductCodes, getAdminCampaigns, getAdminCraftsmen, getAdminDealers, getAdminLoyaltyReport, getAdminLoyaltyRules, getAdminOverview, getAdminProducts, getAdminRewards, getAdminRiskCases, getAdminSupportRequests, getAdminTransactions, getCampaigns, getCraftsmanDashboard, getCraftsmanProfile, getNotifications, getReportExportAudits, getRewardRedemptions, getRewards, getSupportRequests, getWallet, loginAdmin, loginDealer, logoutAdmin, logoutCraftsman, logoutDealer, markAllNotificationsRead, markNotificationRead, redeemProductCode, redeemReward, replyAdminSupportRequest, reportDealerRisk, requestOtpCode, returnDealerProduct, setAdminCampaignActive, setAdminEntityActive, updateAdminLoyaltyRules, updateAdminReward, updateAdminRiskStatus, updateAdminSupportRequest, updateCraftsmanProfile, verifyDealerCoupon, verifyMembershipPass, verifyOtpCode, type AdminLoginResult, type AdminCampaign, type AdminCraftsman, type AdminDealer, type AdminLoyaltyReport, type AdminOverview, type AdminProduct, type AdminReward, type AdminRiskCase, type AdminSupportRequest, type AdminTransactionResponse, type Campaign, type CraftsmanNotification, type CraftsmanProfile, type Dashboard, type DealerCoupon, type DealerLoginResult, type DealerSaleResult, type MembershipPassResult, type LoyaltyRules, type ProductReturnResult, type ReportExportAudit, type Reward, type RewardRedemption, type RewardRedemptionResult, type SupportItem, type Wallet as WalletData } from './api'
import './App.css'
...
function DealerSalePanel() {
  ...
  const save = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setMessage(''); try { const sale = await createDealerSale(membershipToken.trim().toUpperCase(), saleReference.trim().toUpperCase(), totalAmount); setResult(sale); setVerifiedName(''); setMembershipToken(''); setSaleReference(''); setTotalAmount(0) } catch (error) { setMessage(error instanceof Error ? error.message : 'Satış eşleştirilemedi.') } finally { setBusy(false) } }
