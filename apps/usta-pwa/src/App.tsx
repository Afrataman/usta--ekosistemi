...
import { createSupportRequest, fulfillDealerCoupon, getAdminOverview, getAdminRiskCases, getCampaigns, getCraftsmanDashboard, getCraftsmanProfile, getRewardRedemptions, getRewards, getSupportRequests, getWallet, redeemProductCode, redeemReward, reportDealerRisk, requestOtpCode, returnDealerProduct, updateAdminRiskStatus, updateCraftsmanProfile, verifyDealerCoupon, verifyOtpCode, type AdminOverview, type AdminRiskCase, type Campaign, type CraftsmanProfile, type Dashboard, type DealerCoupon, type ProductReturnResult, type Reward, type RewardRedemption, type RewardRedemptionResult, type SupportItem, type Wallet as WalletData } from './api'
import './App.css'
...
function App() { return window.location.pathname.startsWith('/admin') ? <AdminApp /> : window.location.pathname.startsWith('/dealer/risk') ? <DealerRiskPage /> : window.location.pathname.startsWith('/dealer') ? <DealerApp /> : <CraftsmanApp /> }
