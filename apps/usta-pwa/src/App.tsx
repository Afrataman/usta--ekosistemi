...
function AdminPortal() {
  ...
  return <><AdminRouter /><div className="admin-shortcuts"><a href="/admin/notifications">Bildirim Gönder</a><a href="/admin/dealer-employees">Bayi Çalışanları</a><a href="/admin/adjustments">± Puan Düzelt</a></div><button className="admin-logout" onClick={exit} type="button">{profile.user} · Çıkış</button></>
}
...
function DealerActivityPage() {
  const [data, setData] = useState<DealerActivityResponse | null>(null), [filter, setFilter] = useState(''), [message, setMessage] = useState('')
  useEffect(() => { getDealerActivity(filter || undefined).then(setData).catch((error) => setMessage(error instanceof Error ? error.message : 'Geçmiş yüklenemedi.')) }, [filter])
  ...
  return <main className="dealer-history-shell"><header><div><span>⚒</span><div><small>USTA KULÜBÜ</small><strong>Bayi İşlem Geçmişi</strong></div></div><a href="/dealer">İşlemlere dön</a></header>{message && <p className="dealer-message">{message}</p>}<div className="dealer-history-stats"><article><strong>{data?.summary.sales ?? 0}</strong><span>Satış</span></article><article><strong>{data?.summary.coupons ?? 0}</strong><span>Kupon</span></article><article><strong>{data?.summary.returns ?? 0}</strong><span>İade</span></article><article><strong>{data?.summary.risks ?? 0}</strong><span>Risk</span></article></div><nav className="dealer-history-filters">{[['', 'Tümü'], ['Sale', 'Satış'], ['Coupon', 'Kupon'], ['Return', 'İade'], ['Risk', 'Risk']].map(([value, label]) => <button className={filter === value ? 'active' : ''} onClick={() => setFilter(value)} type="button" key={value}>{label}</button>)}</nav><section className="dealer-activity-list">{data?.rows.map((item) => <article key={`${item.type}-${item.id}`}><span className={`activity-icon ${item.type.toLowerCase()}`}>{item.type === 'Sale' ? '₺' : item.type === 'Coupon' ? '▦' : item.type === 'Return' ? '↩' : '⚑'}</span><div><div className="activity-title"><strong>{item.title}</strong><b>{statuses[item.status] ?? item.status}</b></div><p>{item.detail}</p><dl><div><dt>Tür</dt><dd>{labels[item.type]}</dd></div><div><dt>Usta</dt><dd>{item.craftsman}</dd></div><div><dt>Çalışan</dt><dd>{item.employee}</dd></div><div><dt>Referans</dt><dd>{item.reference}</dd></div></dl><time>{dateFormatter.format(new Date(item.occurredAtUtc))}</time></div></article>)}{data && data.rows.length === 0 && <p className="dealer-history-empty">Bu filtrede işlem bulunmuyor.</p>}</section><aside className="dealer-history-note">Yalnızca giriş yaptığınız bayiye bağlı işlemler gösterilir. Referans numarasını destek veya şüpheli işlem bildiriminde kullanabilirsiniz.</aside></main>
}
