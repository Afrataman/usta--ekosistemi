...
function SupportRequestCard({ item, craftsmanId, onChanged }: { item: SupportItem; craftsmanId: string; onChanged: () => Promise<void> }) {
  ...
  return <article className="craftsman-support-card"><button className="support-card-summary" onClick={() => setExpanded(!expanded)} type="button"><span>{item.category}</span><div><h3>{item.subject}</h3><small>{dateFormatter.format(new Date(item.createdAtUtc))} · {item.responses.length} yanıt</small></div><b>{status}</b></button>{expanded && <div className="craftsman-support-thread"><div className="support-original"><b>Siz</b><p>{item.description}</p></div>{item.responses.map((response) => <div className={response.author === 'Usta' ? 'from-craftsman' : 'from-support'} key={response.id}><b>{response.author === 'Usta' ? 'Siz' : response.author}</b><p>{response.message}</p><small>{dateFormatter.format(new Date(response.createdAtUtc))}</small></div>)}{item.status !== 'Closed' && <form onSubmit={send}><textarea value={reply} onChange={(event) => setReply(event.target.value)} minLength={3} maxLength={1500} placeholder="Destek ekibine yanıt yazın…" required /><button disabled={busy} type="submit">{busy ? 'Gönderiliyor…' : 'Yanıtla'}</button></form>}{message && <p className="support-reply-message">{message}</p>}</div>}</article>
}
