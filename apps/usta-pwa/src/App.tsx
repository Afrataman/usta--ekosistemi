...
import { createSupportRequest, fulfillDealerCoupon, getAdminCraftsmen, getAdminDealers, getAdminOverview, getAdminRiskCases, getCampaigns, getCraftsmanDashboard, getCraftsmanProfile, getRewardRedemptions, getRewards, getSupportRequests, getWallet, redeemProductCode, redeemReward, reportDealerRisk, requestOtpCode, returnDealerProduct, setAdminEntityActive, updateAdminRiskStatus, updateCraftsmanProfile, verifyDealerCoupon, verifyOtpCode, type AdminCraftsman, type AdminDealer, type AdminOverview, type AdminRiskCase, type Campaign, type CraftsmanProfile, type Dashboard, type DealerCoupon, type ProductReturnResult, type Reward, type RewardRedemption, type RewardRedemptionResult, type SupportItem, type Wallet as WalletData } from './api'
import './App.css'
...
function App() { const path = window.location.pathname; return path.startsWith('/admin/craftsmen') ? <AdminManagementPage kind="craftsmen" /> : path.startsWith('/admin/dealers') ? <AdminManagementPage kind="dealers" /> : path.startsWith('/admin') ? <AdminApp /> : path.startsWith('/dealer/risk') ? <DealerRiskPage /> : path.startsWith('/dealer') ? <DealerApp /> : <CraftsmanApp /> }
