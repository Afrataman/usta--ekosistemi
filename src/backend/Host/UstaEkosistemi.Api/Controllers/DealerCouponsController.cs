...
public sealed class DealerCouponsController(UstaEkosistemiDbContext dbContext, DealerSessionAuthenticator authenticator) : ControllerBase
{
    [HttpGet("{code}")]
    public async Task<IActionResult> Verify(string code, CancellationToken cancellationToken)
    {
        if (await authenticator.AuthenticateAsync(Request.Headers.Authorization, cancellationToken) is null) return Unauthorized(new { message = "Bayi oturumu geçersiz veya süresi dolmuş." });
    ...
    public async Task<IActionResult> Fulfill(string code, CancellationToken cancellationToken)
    {
