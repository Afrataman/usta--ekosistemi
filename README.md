...
API ile SQL Server bağlantısının hazır olduğunu, veri değiştirmeden kontrol etmek için:

```powershell
.\scripts\test-local-system.ps1
```

Çalışmayı bitirdiğinizde yalnızca bu projeye ait `5028` ve `5173` portlarındaki yerel süreçleri kapatmak için:
